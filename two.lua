...
-- Загружается автоматически из one.lua в одном контексте
-- ============================================================

local FH = (getgenv and getgenv().FH) or _G.FH or rawget(_G, "FH")
if not FH then
    warn("[FH] two.lua: FH не найден. Убедись, что one.lua загрузился первым.")
    return
end
if type(FH.Tabs) ~= "table" then
    warn("[FH] two.lua: FH.Tabs пустой. Проверь порядок в one.lua — экспорт должен быть ПОСЛЕ CreateAllModules.")
    return
end
...
local Addon = {
    ...
    knifeRadius = 12, knifeLead = 1.0, knifeAir = 0.35, knifeOffset = 0,
...
local function BuildUI()
    ...
    if not (tabCombat and tabVisual and tabUtility and tabSettings and tabAutoFarm) then
        logErr("two.lua: некоторые табы не найдены (Combat/Visual/Utility/Settings/AutoFarm)")
        return
    end
    ...
    -- ========== SILENT AIM (в Бой) ==========
    Reg("SilentAim", L("silent_aim") or "Silent Aim")
    Reg("SilentAimMode", L("silent_aim_mode") or "Silent Aim Mode")
    Reg("AutoShoot", L("auto_shoot") or "Auto Shoot")

    tabCombat:AddDropdown("SilentAimMode", {
        Title = L("silent_aim_mode") or "Silent Aim Mode",
        Values = { L("silent_aim_v1") or "v1 (FireServer)", L("silent_aim_v2") or "v2 (Hook)" },
        Default = L("silent_aim_v2") or "v2 (Hook)",
    }):OnChanged(function(v)
        Addon.silentMode = (v == (L("silent_aim_v1") or "v1 (FireServer)")) and "v1" or "v2"
        if Addon.silentMode == "v1" then UninstallSilentHook() else InstallSilentHook() end
    end)
    tabCombat:AddToggle("SilentAim", { Title = L("silent_aim") or "Silent Aim", Default = false }):OnChanged(function(v)
        Addon.silentOn = v
        if v and Addon.silentMode == "v2" then InstallSilentHook() end
    end)
    tabCombat:AddToggle("AutoShoot", { Title = L("auto_shoot") or "Auto Shoot", Default = false }):OnChanged(function(v)
        Addon.autoShootOn = v
    end)
    tabCombat:AddSlider("AutoShootDelay", { Title = L("auto_shoot_delay") or "Delay (ms)", Min = 0, Max = 600, Default = 0, Rounding = 0 }):OnChanged(function(v)
        Addon.autoDelay = v
    end)

    -- ========== KNIFE (в Бой) ==========
    tabCombat:AddToggle("KnifeSilent", { Title = L("knife_silent") or "Silent Throw", Default = false }):OnChanged(function(v) Addon.knifeSilentOn = v end)
    tabCombat:AddToggle("KnifePred", { Title = L("knife_pred") or "Knife Prediction", Default = true }):OnChanged(function(v) Addon.knifePredOn = v end)
    tabCombat:AddSlider("KnifeLead", { Title = L("knife_lead") or "Lead (%)", Min = 0, Max = 320, Default = 100, Rounding = 0 }):OnChanged(function(v) Addon.knifeLead = v / 100 end)
    tabCombat:AddSlider("KnifeAir", { Title = L("knife_air") or "Air lift (%)", Min = 0, Max = 120, Default = 35, Rounding = 0 }):OnChanged(function(v) Addon.knifeAir = v / 100 end)
    tabCombat:AddSlider("KnifeOffset", { Title = L("knife_offset") or "Offset (ms)", Min = -80, Max = 220, Default = 0, Rounding = 0 }):OnChanged(function(v) Addon.knifeOffset = v / 1000 end)
    tabCombat:AddToggle("KnifeInsta", { Title = L("knife_insta") or "Insta Kill", Default = false }):OnChanged(function(v) Addon.knifeInstaOn = v end)
    tabCombat:AddSlider("KnifeRadius", { Title = L("knife_radius") or "Impact radius", Min = 2, Max = 40, Default = 12, Rounding = 0 }):OnChanged(function(v) Addon.knifeRadius = v end)

    -- ========== KILL AURA V2 (в Бой) ==========
    tabCombat:AddDropdown("KillAuraVersion", {
        Title = L("kill_aura_version") or "KillAura version",
        Values = { L("kill_aura_v1") or "v1", L("kill_aura_v2") or "v2" },
        Default = L("kill_aura_v1") or "v1",
    }):OnChanged(function(v)
        if v == (L("kill_aura_v2") or "v2") then
            Addon.kaV2On = true
    ...
    -- ========== BACKTRACK (в Визуал) ==========
    tabVisual:AddToggle("Backtrack", { Title = L("backtrack") or "Backtrack", Default = false }):OnChanged(function(v)
        Addon.btOn = v
        if not v then BtDestroy() end
    end)
    tabVisual:AddColorPicker("BtColor", { Title = L("backtrack_color") or "Backtrack color", Default = Color3.fromRGB(255, 60, 60) }):OnChanged(function(c) Addon.btColor = c end)

    -- ========== TRACER (в Визуал) ==========
    tabVisual:AddToggle("Tracer", { Title = L("tracer") or "Bullet Tracer", Default = false }):OnChanged(function(v) Addon.tracerOn = v end)
    tabVisual:AddColorPicker("TracerColor", { Title = L("tracer_color") or "Tracer color", Default = Color3.fromRGB(133, 220, 255) }):OnChanged(function(c) Addon.tracerColor = c end)
    tabVisual:AddSlider("TracerDur", { Title = L("tracer_duration") or "Duration", Min = 0.1, Max = 5, Default = 1, Rounding = 1 }):OnChanged(function(v) Addon.tracerDur = v end)

    -- ========== CHINA HAT (в Визуал) ==========
    tabVisual:AddToggle("ChinaHat", { Title = L("china_hat") or "China Hat", Default = false }):OnChanged(function(v)
        Addon.chinaHatOn = v
        if not v then ChinaHatClear() end
    end)
    tabVisual:AddColorPicker("ChinaHatColor", { Title = L("china_hat_color") or "Color", Default = Color3.fromRGB(255, 60, 60) }):OnChanged(function(c) Addon.chinaHatColor = c end)

    -- ========== SELF CHAMS (в Визуал) ==========
    tabVisual:AddToggle("SelfChams", { Title = L("self_chams") or "Self Chams", Default = false }):OnChanged(function(v)
        Addon.selfChamsOn = v
        if not v then SelfChamsClear() end
    end)
    tabVisual:AddColorPicker("SelfChamsColor", { Title = L("self_chams_color") or "Color", Default = Color3.fromRGB(120, 60, 255) }):OnChanged(function(c) Addon.selfChamsColor = c end)

    -- ========== MOVEMENT GRAPH (в Визуал) ==========
    tabVisual:AddToggle("MovGraph", { Title = L("mov_graph") or "Movement Graph", Default = false }):OnChanged(function(v)
        Addon.movGraphOn = v
        if not v then MovGraphClear() end
    end)
    tabVisual:AddColorPicker("MovGraphColor", { Title = L("mov_graph_color") or "Color", Default = Color3.fromRGB(242, 242, 242) }):OnChanged(function(c) Addon.movGraphColor = c end)
    tabVisual:AddSlider("MovGraphWidth", { Title = L("mov_graph_width") or "Width", Min = 180, Max = 420, Default = 280, Rounding = 0 }):OnChanged(function(v) Addon.movGraphWidth = v end)
    tabVisual:AddSlider("MovGraphHeight", { Title = L("mov_graph_height") or "Height", Min = 40, Max = 120, Default = 72, Rounding = 0 }):OnChanged(function(v) Addon.movGraphHeight = v end)
    tabVisual:AddSlider("MovGraphY", { Title = L("mov_graph_y") or "Y", Min = -200, Max = 400, Default = 180, Rounding = 0 }):OnChanged(function(v) Addon.movGraphY = v end)

    -- ========== ANTI (в Утилиты) ==========
    tabUtility:AddToggle("AntiFling", { Title = L("anti_fling") or "Anti-Fling", Default = false }):OnChanged(function(v) Addon.antiFlingOn = v end)
    tabUtility:AddToggle("AntiVoid", { Title = L("anti_void") or "Anti-Void", Default = false }):OnChanged(function(v) Addon.antiVoidOn = v end)
    tabUtility:AddToggle("AntiTrap", { Title = L("anti_trap") or "Anti-Trap", Default = false }):OnChanged(function(v) Addon.antiTrapOn = v end)

    -- ========== FAKE POS (в Утилиты) ==========
    tabUtility:AddToggle("FakePos", { Title = L("fake_pos") or "Fake Position", Default = false }):OnChanged(function(v) Addon.fakePosOn = v end)
    tabUtility:AddSlider("FakePosX", { Title = L("fake_pos_x") or "Spread X", Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v) Addon.fakePosX = v * 1e9 end)
    tabUtility:AddSlider("FakePosY", { Title = L("fake_pos_y") or "Spread Y", Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v) Addon.fakePosY = v * 1e9 end)
    tabUtility:AddSlider("FakePosZ", { Title = L("fake_pos_z") or "Spread Z", Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v) Addon.fakePosZ = v * 1e9 end)

    -- ========== TOOLS (в Утилиты) ==========
    tabUtility:AddToggle("TPTool", { Title = L("tp_tool") or "TP Tool", Default = false }):OnChanged(function(v)
        Addon.tpToolOn = v
        if not v then RemoveTPTool() end
    end)
    tabUtility:AddToggle("FlingTool", { Title = L("fling_tool") or "Fling Tool", Default = false }):OnChanged(function(v)
        Addon.flingToolOn = v
        if not v then RemoveFlingTool() end
    end)
    tabUtility:AddToggle("FlingBypass", { Title = L("fling_bypass") or "Fling Bypass Velocity", Default = false }):OnChanged(function(v) Addon.flingBypassVel = v end)

    -- ========== SOUND (в Настройки) ==========
    tabSettings:AddToggle("SoundReplacer", { Title = L("sound_replacer") or "Kill sound", Default = false }):OnChanged(function(v) Addon.soundOn = v end)
    tabSettings:AddDropdown("SoundSheriff", { Title = L("sound_sheriff") or "Sheriff kill", Values = { "mc bow", "neverlose", "rust", "primordial", "sparkle", "break" }, Default = "mc bow" }):OnChanged(function(v) Addon.soundSheriff = v end)
    tabSettings:AddDropdown("SoundMurder", { Title = L("sound_murder") or "Murder kill", Values = { "skeet", "neverlose", "rust", "primordial", "sparkle", "break" }, Default = "skeet" }):OnChanged(function(v) Addon.soundMurder = v end)
    tabSettings:AddSlider("SoundVol", { Title = L("sound_volume") or "Volume", Min = 0.1, Max = 5, Default = 1, Rounding = 1 }):OnChanged(function(v) Addon.soundVol = v end)

    -- ========== AUTOFARM V2 (в Автофарм) ==========
    tabAutoFarm:AddDropdown("AutoFarmVersion", {
        Title = L("autofarm_version") or "AutoFarm version",
        Values = { L("autofarm_v1") or "v1", L("autofarm_v2") or "v2" },
        Default = L("autofarm_v1") or "v1",
    }):OnChanged(function(v)
        Addon.farmV2On = (v == (L("autofarm_v2") or "v2"))
    ...
    tabFling:AddToggle("FlingToggle", {
        ...
        Addon.flingOn = v
        Notify(L("notify_title") or "FortniHub", "Fling " .. (v and "ON" or "OFF"), 1.5)
    end)
    ...
    tabFling:AddButton({
        ...
        Callback = function()
            local targets = GetFlingTargets()
            if #targets == 0 then Notify(L("notify_title") or "FortniHub", "Нет таргета", 2); return end
            FlingPlayer(targets[1])
            Notify(L("notify_title") or "FortniHub", "Fling: " .. targets[1].Name, 2)
        end,
    })
...
if not ok then logErr("two.lua init error: " .. tostring(err)) end

local okUI, errUI = pcall(BuildUI)
if not okUI then logErr("two.lua UI error: " .. tostring(errUI)) else logInfo("two.lua UI собран") end
