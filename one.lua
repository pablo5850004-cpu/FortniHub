...
do
    ...
    kaSec:AddDropdown("KAVersion", {
        ...
        Values = {L("Kill Aura v1 (твоя)"), L("Kill Aura v2 (друга)")},
        Default = L("Kill Aura v2 (друга)"),
    }):OnChanged(function(v)
        killAuraVersion = (v == L("Kill Aura v1 (твоя)")) and "v1" or "v2"
...
-- ============================================================
-- FORTNIHUB v16 — Часть 3/3: Tools, Farm, Anti, Sounds, Map Vote
-- ============================================================

-- ============================================================
-- AUTOFARM — v1 (твой старый) + v2 (друга) с селектором
-- ============================================================
do
    ...
    local farmVersion = "v2" -- по дефолту v2 (правильный)

    farmSec:AddDropdown("FarmVersion", {
        Title = L("AutoFarm Version"),
        Values = {L("AutoFarm v1 (твой)"), L("AutoFarm v2 (друга)")},
        Default = L("AutoFarm v2 (друга)"),
    }):OnChanged(function(v)
        farmVersion = (v == L("AutoFarm v1 (твой)")) and "v1" or "v2"
    ...
    -- ==== V1 (твой, через firetouchinterest + move по CFrame) ====
    AddConn("FarmV1", RunService.Heartbeat:Connect(function(_, dt)
        ...
        if dist > 0.5 then
            local step = math.min(farmV1.speed * dt, dist)
    ...
    -- ==== V2 (друга, через CoinVisual tag + ProximityPrompt) ====
    AddConn("FarmV2", RunService.Heartbeat:Connect(function(_, dt)
        ...
        local dist = dir.Magnitude
        if dist > 0.5 then
            local step = math.min(farmV2.speed * dt, dist)
            hrp.CFrame = CFrame.new(hrp.Position + dir.Unit * step)
            hrp.AssemblyLinearVelocity = Vector3.zero
        end
...
-- ============================================================
-- TOOLS — TP Tool, Fling Tool, Fling Murder/Sheriff
-- ============================================================
do
...
-- ============================================================
-- ANTI — Fling, Void, Trap, Coin, Fade
-- ============================================================
do
        ...
        while task.wait(0.5) do
            if notifyOn and rolesOn then
                ...
                if r and r ~= lastRole then
                    lastRole = r
                    local ru = (r == "Sheriff" or r == "Hero") and "Шериф" or (r == "Murderer" and "Маньяк" or (r == "Innocent" and "Мирный" or r))
                    push("Роль: " .. ru)
                elseif not r then
                    lastRole = nil
                end
...
do
    ...
    local function sndResolve(name)
        ...
        if not f and not sndWarned[name] then
            sndWarned[name] = true
        end
        return f
    end
    ...
    local function sndConsider(inst)
        ...
        if kind then sndHook(inst, kind) end
    end
...
-- ============================================================
do
    local emoteSec = Tabs.Troll:AddSection({Name = L("Emote")})
    ...
    local emoteDrop = emoteSec:AddDropdown("EmoteList", {
        Title = L("Emote"),
        Values = emoteList,
        Default = emoteList[1],
    }):OnChanged(function(v)
    ...
    emoteSec:AddButton({Title = L("Stop Emote"), Callback = stopEmote})
        ...
        if ok and type(res) == "table" then
            ...
            for _, item in pairs(list) do
                ...
                if id and id > 0 and not seen[id] then
                    ...
                    if emoteMap[nm] then nm = nm .. " [" .. id .. "]" end
                    emoteMap[nm] = tostring(id)
                    newList[#newList + 1] = nm
                end
            end
            for _, nm in ipairs(emoteList) do newList[#newList + 1] = nm end
            pcall(function()
                emoteDrop:SetValues(newList)
                emoteDrop:Generate()
            end)
        end
    end)
...
do
    ...
    mvSec:AddSlider("MVDupeCap", {Title = L("Max Dupe"), Min = 1, Max = 10, Default = 3, Rounding = 0}):OnChanged(function(v) dupeCap = v end)
...
-- ============================================================
-- SETTINGS — HUD, Coord, Unload
-- ============================================================
do
    local tS = Window:AddTab({Title = L("Settings")})
    Tabs.Settings = tS
    local setSec = tS:AddSection({Name = "Настройки"})

    setSec:AddToggle("ShowHUD", {Title = L("Show HUD"), Default = true}):OnChanged(function(v)
...
print("[FortniHub][INFO] Фичи: Autofarm v1/v2, Tools (TP/Fling),")
print("[FortniHub][INFO]       Anti (Fling/Void/Trap/Coin/Fade),")
print("[FortniHub][INFO]       Notify, Sounds, Emotes, Map Vote")
print("[FortniHub][INFO] ================================")
print("[FortniHub][INFO] FortniHub v16.0.0 REWRITE — ВСЁ ГОТОВО")
print("[FortniHub][INFO] Меню: P | Minimize: RightControl")

task.spawn(function()
    task.wait(1)
    Notify("FortniHub", "v16 полностью загружен! P - меню", 8)
end)
