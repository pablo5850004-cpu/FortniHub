...
-- AWP: обновлённая детализированная модель (83 части) + мобильная кнопка Shoot
-- ============================================================
...
-- КНОПКА SHOOT (тихий выстрел для мобилы) — УМЕНЬШЕНА, СКРЫТА ПО УМОЛЧАНИЮ
...
local function CreateTouchButton(modName)
    ...
    -- СПЕЦ-КЕЙС: кнопка Shoot (быстрый тихий выстрел)
...
-- ФУНКЦИЯ ТИХОГО ВЫСТРЕЛА (используется и хоткеем, и кнопкой Shoot)
...
-- ============================================================
-- AWP МОДЕЛЬ — новая детализированная версия от Клода
-- ============================================================
...
local function buildAWPModel(handle)
    ...
    for _,def in ipairs(PART_DEFS) do
        ...
        part.CFrame=handle.CFrame*MODEL_CFRAME*def.cframe
        part.Parent=tool
        AWP_weldTo(handle,part)
        table.insert(AWPCreatedParts,part)
    end
...
local function CreateAllModules()
    ...
    -- ★ ShowShootBtn перенесён сюда из Настроек ★
    ...
    -- (ShowShootBtn тут БОЛЬШЕ НЕТ — он теперь в Мобильном табе)
...
-- BIND HANDLER — используем общую DoQuietShot
...
-- КНОПКА SHOOT на мобиле (основная, в правом нижнем углу)
