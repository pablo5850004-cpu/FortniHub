-- ============================================================
-- FORTNIHUB v14.1.0 — MM2 FULL EXPLOIT HUB — by HOTI
-- AWP fix + QuietShot aim modes + SafeLoader
-- Fixed: KillAura waits knife in hand, AutoFarm rewritten
-- Merged addon tabs into main tabs
-- New visuals: China Hat, Self Chams, Movement Graph, Tracer, Backtrack
-- ============================================================
...
local L_ru = {
    ...
    -- addon keys (все в существующих табах)
    ...
    mov_graph_width="Ширина", mov_graph_height="Высота", mov_graph_y="Позиция Y",
}
local L_en = {}
...
local function safeHttpGet(url, name)
    if type(url) ~= "string" or url == "" then
        logWarn("safeLoad: пустой URL для " .. tostring(name))
        return nil
    end
    local ok, body = pcall(function() return game:HttpGet(url, true) end)
    if not ok then
        logWarn("safeLoad: HTTP ошибка для " .. tostring(name) .. ": " .. tostring(body))
        return nil
    end
    if type(body) ~= "string" then return nil end
    if #body < 32 then
        logWarn("safeLoad: слишком короткий ответ для " .. tostring(name))
        return nil
    end
    local head = body:sub(1, 200):lower()
    if head:find("<!doctype") or head:find("<html") or head:find("not found") then
        logWarn("safeLoad: сервер отдал HTML/404 для " .. tostring(name))
        return nil
    end
    return body
end
local function safeLoadstring(body, name)
    ...
    if type(fn) ~= "function" then
        logWarn("safeLoadstring: синтаксис сломан в " .. tostring(name))
        return nil
    end
    return fn
end
local function safeRun(fn, name)
    ...
    if not ok then
        logWarn("safeRun: ошибка выполнения " .. tostring(name) .. ": " .. tostring(res))
        return nil
    end
    return res
end
...
do
    ...
    if fn then
        ...
        if res ~= nil then TouchFlingLoaded = true end
    end
end
...
do
    ...
    if fn then safeRun(fn, "Invisible") end
end
task.wait(0.5)
...
for _, url in ipairs(fluentUrls) do
    ...
    if body and safeLoadstring(body, "Fluent") then
        fluentBody = body
        break
    end
end
if not fluentBody then
    logErr("Не удалось загрузить Fluent UI!")
    return
end
Fluent = safeRun(safeLoadstring(fluentBody, "Fluent"), "Fluent")
if type(Fluent) ~= "table" then
    logErr("Fluent вернул не таблицу")
    return
end
...
local function AWP_weldTo(p0,p1)
    local w=Instance.new("WeldConstraint"); w.Part0=p0; w.Part1=p1; w.Parent=p0
end
...
local function buildAWPModel(handle)
    local tool=handle.Parent; if not tool then return end
    ...
    for _,part in ipairs(built) do AWP_weldTo(handle, part) end
...
-- ============================================================
-- CreateAllModules — Silent/Knife в Бой, Анти/Тулы в Утилиты, Визуалы в Визуал
-- ============================================================
local function CreateAllModules()
    ...
    tabCombat:AddToggle("ForceShoot", { Title = L("force_shoot"), Default = false }):OnChanged(function(v)
        Addon.forceShootOn = v
        if not v then AddonRestoreForceOrigin() end
    end)
    tabCombat:AddSlider("ForceStandOff", { Title = L("force_stand_off"), Min = 0, Max = 40, Default = 15, Rounding = 0 }):OnChanged(function(v)
        Addon.forceStandOff = v
    end)

    -- Нож
    tabCombat:AddToggle("KnifeSilent", { Title = L("knife_silent"), Default = false }):OnChanged(function(v)
        Addon.knifeSilentOn = v
    end)
    tabCombat:AddToggle("KnifePred", { Title = L("knife_pred"), Default = true }):OnChanged(function(v)
        Addon.knifePredOn = v
    end)
    tabCombat:AddSlider("KnifeLead", { Title = L("knife_lead"), Min = 0, Max = 320, Default = 100, Rounding = 0 }):OnChanged(function(v)
        Addon.knifeLead = v / 100
    end)
    tabCombat:AddSlider("KnifeAir", { Title = L("knife_air"), Min = 0, Max = 120, Default = 35, Rounding = 0 }):OnChanged(function(v)
        Addon.knifeAir = v / 100
    end)
    tabCombat:AddSlider("KnifeOffset", { Title = L("knife_offset"), Min = -80, Max = 220, Default = 0, Rounding = 0 }):OnChanged(function(v)
        Addon.knifeOffset = v / 1000
    end)
    tabCombat:AddSlider("KnifeSpeed", { Title = L("knife_speed"), Min = 40, Max = 400, Default = 96, Rounding = 0 }):OnChanged(function(v)
        Addon.knifeSpeed = v
    end)
    tabCombat:AddToggle("KnifeInsta", { Title = L("knife_insta"), Default = false }):OnChanged(function(v)
        Addon.knifeInstaOn = v
    end)
    tabCombat:AddSlider("KnifeRadius", { Title = L("knife_radius"), Min = 2, Max = 40, Default = 12, Rounding = 0 }):OnChanged(function(v)
        Addon.knifeRadius = v
    end)
    ...
    tabAutoFarm:AddDropdown("AutoFarmVersion", {
        ...
        -- Меняем ТОЛЬКО режим. Тумблер остаётся выключен пока юзер не включит.
    ...
    tabAutoFarm:AddToggle("AutoFarmCoins",{Title=L("autofarm"),Default=false}):OnChanged(function(v)
        ...
        if v then
            BlockModulesForFarm()
            FarmLastCoinTime=tick()
            if Addon.farmV2On then
                -- v2 обрабатывается в своём цикле
            else
                FarmRunning=false
                StartFarm()
            end
        else
            UnblockModulesFromFarm()
            StopFarm()
        end
    end)
    ...
    Reg("Backtrack", L("backtrack")); Reg("Tracer", L("tracer"))
    ...
    tabVisual:AddColorPicker("BtColor", { Title = L("backtrack_color"), Default = Color3.fromRGB(255, 60, 60) }):OnChanged(function(c)
        Addon.btColor = c
    end)

    -- Tracer
    tabVisual:AddToggle("Tracer", { Title = L("tracer"), Default = false }):OnChanged(function(v)
        Addon.tracerOn = v
    end)
    tabVisual:AddColorPicker("TracerColor", { Title = L("tracer_color"), Default = Color3.fromRGB(133, 220, 255) }):OnChanged(function(c)
        Addon.tracerColor = c
    end)
    tabVisual:AddSlider("TracerDur", { Title = L("tracer_duration"), Min = 0.1, Max = 5, Default = 1, Rounding = 1 }):OnChanged(function(v)
        Addon.tracerDur = v
    end)
    ...
    tabVisual:AddToggle("ChinaHat", { Title = L("china_hat"), Default = false }):OnChanged(function(v)
        Addon.chinaHatOn = v
        if not v then AddonChinaHatClear() end
    end)
    tabVisual:AddColorPicker("ChinaHatColor", { Title = L("china_hat_color"), Default = Color3.fromRGB(255, 60, 60) }):OnChanged(function(c)
        Addon.chinaHatColor = c
    end)

    -- Self Chams
    tabVisual:AddToggle("SelfChams", { Title = L("self_chams"), Default = false }):OnChanged(function(v)
        Addon.selfChamsOn = v
        if not v then AddonSelfChamsClear() end
    end)
    tabVisual:AddColorPicker("SelfChamsColor", { Title = L("self_chams_color"), Default = Color3.fromRGB(120, 60, 255) }):OnChanged(function(c)
        Addon.selfChamsColor = c
    end)

    -- Movement Graph
    tabVisual:AddToggle("MovGraph", { Title = L("mov_graph"), Default = false }):OnChanged(function(v)
        Addon.movGraphOn = v
        if not v then AddonMovGraphClear() end
    end)
    tabVisual:AddColorPicker("MovGraphColor", { Title = L("mov_graph_color"), Default = Color3.fromRGB(242, 242, 242) }):OnChanged(function(c)
        Addon.movGraphColor = c
    end)
    tabVisual:AddSlider("MovGraphWidth", { Title = L("mov_graph_width"), Min = 180, Max = 420, Default = 280, Rounding = 0 }):OnChanged(function(v)
        Addon.movGraphWidth = v
    end)
    tabVisual:AddSlider("MovGraphHeight", { Title = L("mov_graph_height"), Min = 40, Max = 120, Default = 72, Rounding = 0 }):OnChanged(function(v)
        Addon.movGraphHeight = v
    end)
    tabVisual:AddSlider("MovGraphY", { Title = L("mov_graph_y"), Min = -200, Max = 400, Default = 180, Rounding = 0 }):OnChanged(function(v)
        Addon.movGraphY = v
    end)
    ...
    Reg("SoundReplacer", L("sound_replacer"))
    ...
    -- Anti
    tabUtility:AddToggle("AntiFling", { Title = L("anti_fling"), Default = false }):OnChanged(function(v)
        Addon.antiFlingOn = v
    end)
    tabUtility:AddToggle("AntiVoid", { Title = L("anti_void"), Default = false }):OnChanged(function(v)
        Addon.antiVoidOn = v
    end)
    tabUtility:AddToggle("AntiTrap", { Title = L("anti_trap"), Default = false }):OnChanged(function(v)
        Addon.antiTrapOn = v
    end)
    tabUtility:AddToggle("FakePos", { Title = L("fake_pos"), Default = false }):OnChanged(function(v)
        Addon.fakePosOn = v
    end)
    tabUtility:AddSlider("FakePosX", { Title = L("fake_pos_x"), Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v)
        Addon.fakePosX = v * 1e9
    end)
    tabUtility:AddSlider("FakePosY", { Title = L("fake_pos_y"), Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v)
        Addon.fakePosY = v * 1e9
    end)
    tabUtility:AddSlider("FakePosZ", { Title = L("fake_pos_z"), Min = 1, Max = 9, Default = 9, Rounding = 0 }):OnChanged(function(v)
    ...
    tabUtility:AddToggle("FlingBypass", { Title = L("fling_bypass"), Default = false }):OnChanged(function(v)
    ...
    local bindValues={
        ...
        "SilentAim","KnifeSilent","Backtrack","Tracer","ChinaHat","SelfChams",
    }
    ...
    -- Sound Replacer
    tabSettings:AddToggle("SoundReplacer", { Title = L("sound_replacer"), Default = false }):OnChanged(function(v)
        Addon.soundOn = v
    end)
    tabSettings:AddDropdown("SoundSheriff", {
        Title = L("sound_sheriff"),
        Values = { "mc bow", "neverlose", "rust", "primordial", "sparkle", "break" },
        Default = "mc bow",
    }):OnChanged(function(v) Addon.soundSheriff = v end)
    tabSettings:AddDropdown("SoundMurder", {
        Title = L("sound_murder"),
        Values = { "skeet", "neverlose", "rust", "primordial", "sparkle", "break" },
        Default = "skeet",
    }):OnChanged(function(v) Addon.soundMurder = v end)
    tabSettings:AddSlider("SoundVol", { Title = L("sound_volume"), Min = 0.1, Max = 5, Default = 1, Rounding = 1 }):OnChanged(function(v)
        Addon.soundVol = v
    end)
...
-- ============================================================
-- Addon state (глобально, не в функции)
-- ============================================================
Addon = Addon or {
    ...
    knifeTrackers = {}, knifePending = nil, knifeOriginalKnifeAim = nil,
    kaV2On = false, kaV2Dist = 30,
    farmV2On = false, farmDownDepth = 14,
    btOn = false, btColor = Color3.fromRGB(255, 60, 60),
    btModel = nil, btHistory = {}, btCap = 256, btFirst = 1, btCount = 0,
...
local function AddonFindMurderer()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            local c = p.Character
            if c:FindFirstChild("Knife") then return p end
        end
    end
    return nil
end
...
-- KillAura v2 — БЕЗ АВТОЭКИПА (ждёт нож в руке)
AddConnection("AddonKAv2", RunService.Heartbeat:Connect(function()
    if not Addon.kaV2On then return end
    if not AddonAmMurderer() then return end
    -- Проверяем что нож В РУКЕ, не в бэкпаке
    local char = LocalPlayer.Character
    if not char then return end
    local knife = char:FindFirstChild("Knife")
    if not knife then return end  -- ждём пока экипируют вручную
    local events = knife:FindFirstChild("Events")
    if not events then return end
    local stab = events:FindFirstChild("KnifeStabbed")
    local touch = events:FindFirstChild("HandleTouched")
...
-- China Hat
local function AddonChinaHatClear()
    for _, p in ipairs(Addon.chinaHatParts) do
        if p and p.Parent then pcall(function() p:Destroy() end) end
    end
    Addon.chinaHatParts = {}
end
AddConnection("AddonChinaHat", RunService.Heartbeat:Connect(function()
    if not Addon.chinaHatOn then
        if #Addon.chinaHatParts > 0 then AddonChinaHatClear() end
        return
    end
    local char = LocalPlayer.Character
    ...
    if #Addon.chinaHatParts == 0 then
        ...
        for i = 1, segments do
            ...
            local r1 = radius
            local r2 = radius * 0.15
            local x1, z1 = math.cos(angle) * r1, math.sin(angle) * r1
            local x2, z2 = math.cos(nextAngle) * r2, math.sin(nextAngle) * r2
            local x1n, z1n = math.cos(nextAngle) * r1, math.sin(nextAngle) * r1
            local x2n, z2n = math.cos(angle) * r2, math.sin(angle) * r2
            local p = Instance.new("WedgePart")
            p.Anchored = false
            p.CanCollide = false
            p.CanQuery = false
            p.Massless = true
            p.Color = Addon.chinaHatColor
        ...
        -- обновляем цвет на лету
        for _, p in ipairs(Addon.chinaHatParts) do
            if p.Parent and p.Color ~= Addon.chinaHatColor then
                p.Color = Addon.chinaHatColor
            end
        end
    end
...
-- Self Chams
local function AddonSelfChamsClear()
...
AddConnection("AddonSelfChams", RunService.Heartbeat:Connect(function()
    if not Addon.selfChamsOn then
        if next(Addon.selfChamsCache) then AddonSelfChamsClear() end
        return
    end
    local char = LocalPlayer.Character
    if not char then return end
    for _, p in ipairs(char:GetDescendants()) do
        if p:IsA("BasePart") and p.Name ~= "HumanoidRootPart" then
            ...
            if p.Color ~= Addon.selfChamsColor then
                pcall(function() p.Color = Addon.selfChamsColor end)
            end
        end
    end
end))
...
-- Movement Graph
local function AddonMovGraphClear()
    for i = 1, #Addon.movGraphLines do
        pcall(function() Addon.movGraphLines[i]:Remove() end)
    end
    for i = 1, #Addon.movGraphShadows do
        pcall(function() Addon.movGraphShadows[i]:Remove() end)
    end
    if Addon.movGraphText then pcall(function() Addon.movGraphText:Remove() end) end
    Addon.movGraphLines = {}
    Addon.movGraphShadows = {}
    Addon.movGraphText = nil
    Addon.movGraphHistory = {}
end
AddConnection("AddonMovGraph", RunService.RenderStepped:Connect(function(dt)
    ...
    speed = math.sqrt(speed.X * speed.X + speed.Z * speed.Z)
    local now = os.clock()
    ...
    while #Addon.movGraphHistory > 0 and now - Addon.movGraphHistory[1].t > 3 do
        table.remove(Addon.movGraphHistory, 1)
    end
    local w, h = Addon.movGraphWidth, Addon.movGraphHeight
    local center = Camera.ViewportSize
    local x0 = center.X * 0.5 - w * 0.5
    local y0 = center.Y * 0.5 + Addon.movGraphY - h * 0.5
    local maxSpeed = 30
    for _, s in ipairs(Addon.movGraphHistory) do
        if s.v > maxSpeed then maxSpeed = s.v end
    end
    maxSpeed = math.max(maxSpeed, 20)
    if not Addon.movGraphText then
        Addon.movGraphText = Drawing.new("Text")
        Addon.movGraphText.Size = 14
        Addon.movGraphText.Outline = true
    end
    ...
    for i = 1, #Addon.movGraphLines do
        if i < #hist then
            local a = hist[i]; local b = hist[i+1]
...
-- ============================================================
-- AutoFarm — REWRITTEN
-- v1: обычный, v2: Down под карту. Работает ТОЛЬКО при тумблере ON
-- ============================================================
local FarmCheckTimer = 0
local function DoAutoFarmStep(dt)
    if not Options.AutoFarmCoins or not Options.AutoFarmCoins.Value then
    ...
    if cur > FarmLastCoinCount then
        FarmLastCoinCount = cur
        FarmTimeoutStart = tick()
    end
    ...
    if cur >= 40 then
        ...
        Notify(L("notify_title"), L("farm_full"), 3)
        task.wait(0.8)
    end
    ...
    for _, c in ipairs(coins) do
        if c.Parent and not RecentlyTouchedCoins[c] then
            local d = (myHRP.Position - c.Position).Magnitude
    ...
    -- режим v2: телепорт под карту
    local targetPos
    if Addon.farmV2On then
        targetPos = Vector3.new(closest.Position.X, closest.Position.Y - Addon.farmDownDepth, closest.Position.Z)
    else
        targetPos = closest.Position
    end

    -- Anti-stuck: если позиция не меняется долго, перекидываем чуть ближе
    if FarmLastPos then
        local d = (hrp.Position - FarmLastPos).Magnitude
        if d < 0.5 then
            FarmStuck = FarmStuck + dt
        else
            FarmStuck = 0
        end
    end
    ...
    -- движение: телепорт-шагами (плавно)
    local dist = (targetPos - hrp.Position).Magnitude
    if dist > 0.5 then
        local sp = Options.AutoFarmSpeed and Options.AutoFarmSpeed.Value or 8
        local stepDist = math.min(sp * dt * 8, dist)
        local newPos = hrp.Position + (targetPos - hrp.Position).Unit * stepDist
        pcall(function()
            hrp.CFrame = CFrame.new(newPos, newPos + (targetPos - newPos))
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.AssemblyAngularVelocity = Vector3.zero
        end)
    end
    ...
    -- сбор монеты
    if dist <= 6 or FarmStuck > 1.5 then
        pcall(function()
            firetouchinterest(hrp, closest, 0)
            firetouchinterest(hrp, closest, 1)
        end)
        RecentlyTouchedCoins[closest] = tick()
        FarmStuck = 0
    end
    ...
    for coin, t in pairs(RecentlyTouchedCoins) do
        if now - t > 6 or not coin.Parent then
            RecentlyTouchedCoins[coin] = nil
        end
    end
end

AddConnection("FarmV2Loop", RunService.Stepped:Connect(function(_, dt)
    -- AutoFarm v2 (или v1 — оба через эту функцию теперь)
    if Options.AutoFarmCoins and Options.AutoFarmCoins.Value then
        FarmCheckTimer = FarmCheckTimer + dt
        if FarmCheckTimer < 0.016 then return end
        FarmCheckTimer = 0
        pcall(function() DoAutoFarmStep(dt) end)
    end
end))
...
AddConnection("MainLoop",RunService.Heartbeat:Connect(function()
    ...
    -- KILL AURA v1 — только когда нож в руке
...
-- заглушка: старая StartFarm не используется, оставлена для совместимости с toggle
local function StartFarm()
    -- nothing: вся работа в AddConnection("FarmV2Loop")
end
...
AddConnection("CharAdded",LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if Addon.chinaHatOn then AddonChinaHatClear() end
    if hrp then
        hrp.Anchored=false
