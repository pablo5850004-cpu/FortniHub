...
-- AWP: фикс масштаба и велдов
-- QuietShot: два режима аима
-- SafeLoader: безопасная загрузка внешних зависимостей
-- AddonPack v1 (обёрнут в __InitAddon для обхода лимита 200 локалок)
-- ============================================================
...
local function safeHttpGet(url, name)
    ...
    if type(body) ~= "string" then
        logWarn("safeLoad: не строка для " .. tostring(name))
        return nil
    end
    if #body < 32 then
        logWarn("safeLoad: слишком короткий ответ для " .. tostring(name) .. " (" .. #body .. " байт)")
        return nil
    end
...
local function safeLoadstring(body, name)
    ...
    if type(fn) ~= "function" then
        logWarn("safeLoadstring: синтаксис сломан в " .. tostring(name) .. ": " .. tostring(err))
        return nil
    end
    return fn
end
...
do
    ...
    if fn then
        ...
        if res ~= nil then
            TouchFlingLoaded = true
            logInfo("Touch-Fling загружен")
        else
            logWarn("Touch-Fling не выполнился")
        end
    else
...
if not fluentBody then
    logErr("Не удалось загрузить Fluent UI ни с одного зеркала!")
    return
end
local fluentFn = safeLoadstring(fluentBody, "Fluent")
Fluent = safeRun(fluentFn, "Fluent")
if type(Fluent) ~= "table" then
    logErr("Fluent вернул не таблицу, а " .. type(Fluent))
    return
end
...
-- ============================================================================
-- ============================ ADDON PACK v1 =================================
-- ==================== ОБЁРНУТ В ФУНКЦИЮ (лимит локалок) =====================
-- ============================================================================

local function __InitAddon()
-- Локализация аддона
do
    local adds = {
        ru = {
...
local Addon = {
    ...
    farmV2On = false, farmDownDepth = 14, farmV2Mode = "Down", farmV2NoclipCache = {},
    ...
    antiFlingCache = {}, antiFlingConns = {},
    antiVoidOrig = workspace.FallenPartsDestroyHeight,
    antiTrapWindow = 0, antiTrapSpeedCache = 16, antiTrapJumpCache = 50,
    tpToolOn = false, flingToolOn = false, flingBypassVel = false,
    tpToolObj = nil, flingToolObj = nil, flingActive = 0,
    fakePosOn = false, fakePosX = 9e9, fakePosY = 9e9, fakePosZ = 9e9, fakePosMarker = true,
